...
You can specify additional arguments while using `colcon build` to increase compiling speed. Adding `--packages-select MY_PACKAGE` allows you to recompile one specific package. Adding `--symlink-install` creates a symbolic link between currently existing files in `~/auvc_ws/src/YOUR_PACKAGE` and `~/auvc_ws/install`. If you use this argument, currently existing files AND ONLY those files will not need to be recompiled. If you do add or remove files, recompile.
