...
- If the executable is not found, try `source ~/.zshrc` and then `chmod +x /path/to/executable`.
...
This section provides an overview of an offical [ROS2 tutorial](https://docs.ros.org/en/jazzy/Tutorials/Beginner-Client-Libraries/Writing-A-Simple-Py-Publisher-And-Subscriber.html) for writing a simple publisher and subscriber with some modifications. Install and build our introduction nodes:
