...
Save and exit the `publisher.py` file. Now open `subscriber.py`. In this node, we want to subscribe to messages published by `publisher.py` and print the corresponding unit vector to the terminal. We start with the same imports used in the publisher node. Then, define your class and constructor:
...
Here, instead of having the node perform a single task like the publisher node, we use 'rclpy.spin` to keep the node active.
