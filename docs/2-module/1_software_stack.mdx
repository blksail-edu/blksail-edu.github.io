...
ROS2 (Robot Operating System 2) is a set of open-source libraries and tools used to build robotics applications. It is commonly used in robotics research and industry. ROS2 is a middleware that acts as a layer between the operating system and executables that we code. We'll be using a distribution of ROS2 called Jazzy Jalisco.
...
Create a `launch` file in Python3 that starts both the `publisher` and `subscriber` nodes. You can read more about launch files [here](https://docs.ros.org/en/jazzy/Tutorials/Intermediate/Launch/Launch-Main.html).
