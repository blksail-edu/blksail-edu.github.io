...
We will be using Gazebo Harmonic, the latest version of Gazebo (as of 2024).
...
We install the gcc-arm cross-compiler next:
...
Open the `.zshrc` file with your favorite text editor and add the following line:

```bash
PATH=$PATH:$HOME/ardupilot/TARGET_DIR/gcc-arm-none-eabi-10-2020-q4-major/bin
```

Setup permissions and ccache:
...
### Testing the ArduSub Installation

With QGroundControl running, start the `SITL` in the `backseat`:
...
With QGroundControl open and the Gazebo simulation running, start the `SITL` in the `backseat`:
...
You should now see the ROV in QGroundControl. Commands performed by the `SITL` in QGroundControl should be reflected in the Gazebo simulation, subject to the environmental constraints (pool walls and floor).
