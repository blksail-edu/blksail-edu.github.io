...
```zsh
sudo apt install ros-jazzy-ros-gz-bridge libgz-transport13-dev libgz-msgs10-dev libgz-common5-dev libgz-tools2-dev
```
...
You can test that this is working by running the Gazebo `./launch_1x_bluerov.sh` simulation in a `WSL` terminal, and then running the following command from your Raspberry Pi:
