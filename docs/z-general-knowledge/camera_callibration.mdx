...
- A printed checkerboard calibration target (e.g., 7×6 inner corners, 30 mm square size)
...
- Check that ROS 2 is installed:
   ...
   ros2 --version
   ```
- Confirm that the camera is publishing to `/camera/image_raw`
...
1. **Print the calibration target**
   ...
   Print a checkerboard pattern with known dimensions. For example, a 7×6 checkerboard with 30 mm squares. Mount it flat and keep it rigid.
